...
(async () => {
  ...
  await page.waitForSelector(`a[href="javascript:__doPostBack('GridView1','Sort$verifydate_desc')"]`);
  // build result
  let pages = await page.evaluate(() => parseInt(document.querySelector(`#lblCount`).innerText.match(/共(\d+)頁/)[1]))
  let items = await page.evaluate(() => parseInt(document.querySelector(`#lblCount`).innerText.match(/ (\d+)筆紀錄/)[1]))
  if (!FETCH_ALL) {
    pages = 3
    items = 30
  }
    ...
    for (let linkId of linkIds) {
      ...
      let data = await page.evaluate(() => {
        ...
        document.querySelectorAll(`[id^="FileList"]`).forEach(x => {
          ...
          let category = x.parentElement.parentElement.parentElement.querySelector('td:nth-child(1)').innerText.replace(/：$/, '') || '其他'
